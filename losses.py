...
class total_loss(nn.Module):
  def __init__(self, batch_size, loss_weight, temperature, image_vector, text_vector):
  ...
  def forward(self):
    ...
    image_to_text_cosine_similarity_score = cosine_similarity(self.image_vector, self.text_vector)

    text_to_image_cosine_similarity_score = cosine_similarity(self.text_vector, self.image_vector)

    image_to_text_loss = -torch.log((torch.exp(image_to_text_cosine_similarity_score / self.temperature)) /
                            torch.sum(torch.exp(image_to_text_cosine_similarity_score / self.temperature)))

    text_to_image_loss = -torch.log((torch.exp(text_to_image_cosine_similarity_score / self.temperature)) /
                              torch.sum(torch.exp(text_to_image_cosine_similarity_score / self.temperature)))

    total_loss = (1/self.batch_size) * torch.sum(self.loss_weight * image_to_text_loss + (1 - self.loss_weight) * text_to_image_loss)
